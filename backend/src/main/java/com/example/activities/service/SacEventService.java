...
public class SacEventService {
    ...
    private void applyRequest(Activity activity, SacEventRequest request) {
        ...
        // ✅ FIXED: LocalDate → String
        if (request.getDate() != null)
            activity.setDate(request.getDate().format(DATE_FORMAT));

        // ✅ FIXED: LocalTime → String
        if (request.getStartTime() != null)
            activity.setStartTime(request.getStartTime().format(TIME_FORMAT));

        if (request.getEndTime() != null)
            activity.setEndTime(request.getEndTime().format(TIME_FORMAT));
